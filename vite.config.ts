    ...
    test: {
      ...
      coverage: {
        ...
        exclude: [
          ...
          "src/models/**",
          "src/types/**",
          // Configuración declarativa sin reglas de negocio ejecutables.
          "src/config/theme.config.ts",
          "src/config/theme/**",
          "src/config/design-tokens.ts",
          "src/config/constants.ts",
          "src/**/*.styles.{ts,tsx}",
          // Pantallas deliberadamente desconectadas de APIs reales.
          "src/pages/DemoPage.tsx",
          "src/pages/ModuleUnavailablePage.tsx",
        ...
        // Línea base global medida el 2026-08-25 (14.57% statements/lines,
        // 55.76% branches y 51% functions). Se redondea hacia abajo para
        // absorber diferencias mínimas entre plataformas sin permitir que la
        // cobertura retroceda por debajo de la base actual.
        thresholds: {
          statements: 14,
          branches: 55,
          functions: 51,
      ...
      rollupOptions: {
        output: {
          manualChunks(id) {
            ...
            const packageId = normalizedId.slice(
              normalizedId.lastIndexOf("/node_modules/") + 14,
            );
            if (packageId.includes("pdfmake/build/vfs_fonts"))
              return "pdf-fonts";
            ...
            if (
              /^(canvg|dompurify|fflate|fast-png|svg-pathdata)\//.test(
                packageId,
              )
            )
              return "pdf-support";
            ...
            if (packageId.startsWith("@mui/x-data-grid/"))
              return "mui-data-grid";
            if (packageId.startsWith("@mui/x-date-pickers/"))
              return "mui-date-pickers";
            if (packageId.startsWith("@mui/icons-material/"))
              return "mui-icons";
            ...
            if (/^(react|react-dom|scheduler|react-is)\//.test(packageId))
              return "react";
            if (
              packageId.startsWith("react-hook-form/") ||
              packageId.startsWith("@hookform/")
            )
              return "forms";
            if (
              packageId.startsWith("date-fns/") ||
              packageId.startsWith("dayjs/")
            )
              return "dates";
            if (
              packageId.startsWith("lucide-react/") ||
              packageId.startsWith("react-icons/")
            )
              return "icon-libs";
            if (packageId.startsWith("zod/") || packageId.startsWith("yup/"))
              return "validation";
            return "vendor";
          },
        },
