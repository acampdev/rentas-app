...
import { asignacionService, type CreateAsignacionAPIDTO } from './asignacionService';
...
describe('AsignacionService write contract', () => {
  ...
  it.each([
    ...
    ['actualizar', () => asignacionService.actualizarAsignacionAPI(legacyPayload)],
    ['desasignar', () => asignacionService.desasignarAPI(legacyPayload)]
  ])('omits year, status and pensioner fields when %s', async (_name, execute) => {
    const fetchMock = vi.fn().mockResolvedValue(Response.json({ success: true, data: payload }));
    ...
    expect(body).not.toHaveProperty('anio');
