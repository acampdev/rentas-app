...
export const normalizarAsignacion = (
  item: unknown,
  index = 0,
): AsignacionPredio => {
  const raw = toAsignacionRecord(item);
  const porcentaje =
    raw.porcentajeCondomino == null ? null : Number(raw.porcentajeCondomino);
  ...
  const codPredioContribuyente =
    raw.codPredioContribuyente == null
      ? null
      : Number(raw.codPredioContribuyente);
  const codAsignacion =
    raw.codAsignacion == null ? null : (raw.codAsignacion as number | string);
  ...
  return {
    ...
    porcentajeCondominoDesc:
      String(raw.porcentajeCondominoDesc || "").trim() ||
      `${formatPercentage(porcentaje ?? 100)} %`,
    fechaDeclaracion: String(raw.fechaDeclaracion || "").trim(),
    fechaVenta: String(raw.fechaVenta || "").trim(),
    fechaDeclaracionStr: String(
      raw.fechaDeclaracionStr || raw.fechaDeclaracion || "",
    ).trim(),
    ...
    codEstado,
    estado:
      String(raw.estado || "").trim() ||
      (codEstado === "0201" ? "ACTIVO" : "INACTIVO"),
    codUsuario: raw.codUsuario == null ? null : Number(raw.codUsuario),
    nombreContribuyente: String(
      raw.nombreContribuyente || raw.contribuyente || "",
    ).trim(),
