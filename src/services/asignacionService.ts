...
class AsignacionService {
  ...
  async buscarAsignaciones(
    params: AsignacionQueryParams,
  ): Promise<AsignacionPredio[]> {
    const query = new URLSearchParams();
    if (params.anio !== undefined && params.anio !== "") {
      query.set("anio", String(params.anio));
    }
    if (
      params.codContribuyente !== undefined &&
      params.codContribuyente !== ""
    ) {
    ...
    if (response.success === false) {
      throw new Error(
        getAsignacionErrorMessage(
          payload,
          "No se pudieron listar las asignaciones",
        ),
      );
    }
  ...
  private async guardarAsignacion(
    ...
    if (response.success === false) {
      throw new Error(
        getAsignacionErrorMessage(payload, "No se pudo guardar la asignación"),
      );
    }
  ...
  async desasignarAPI(
    ...
    if (response.success === false) {
      throw new Error(
        getAsignacionErrorMessage(payload, "No se pudo desasignar el predio"),
      );
    }
    return {
      success: response.success !== false,
      message: String(
        response.message || "Desasignación realizada correctamente",
      ),
      data: response.data ?? payload,
    };
  }

  async prevalidarBeneficioPensionista(
    codContribuyente: number | string,
  ): Promise<PrevalidacionBeneficio> {
    return this.prevalidarBeneficio(
      "prevalidarBeneficioPensionista",
      codContribuyente,
    );
  }

  async prevalidarBeneficioAdultoMayor(
    codContribuyente: number | string,
  ): Promise<PrevalidacionBeneficio> {
    return this.prevalidarBeneficio(
  ...
  private async prevalidarBeneficio(
    endpoint: string,
    codContribuyente: number | string,
  ): Promise<PrevalidacionBeneficio> {
    const query = new URLSearchParams({
    ...
    const payload = await this.request(
      `${this.endpoint}/${endpoint}?${query.toString()}`,
      { method: "GET" },
    );
    const response = toAsignacionRecord(payload);
    return {
