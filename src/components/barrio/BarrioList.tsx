  ...
  const sortedAndFilteredBarrios = useMemo(() => {
    ...
    filteredData.sort((a, b) => {
      ...
      if (orderBy === 'sector') {
        ...
        aValue = a[orderBy as keyof Barrio] as string | number | undefined | null;
        bValue = b[orderBy as keyof Barrio] as string | number | undefined | null;
      }
                ...
                paginatedBarrios.map((barrio, index) => (
                  <Fade in={true} key={barrio.id} timeout={300 + (index * 100)}>
                    <TableRow
                      ...
                      onClick={() => (onSelectBarrio || onSelect || onEdit)?.(barrio)}
                      sx={{ 
                        cursor: 'pointer',
