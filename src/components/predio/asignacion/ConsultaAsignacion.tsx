...
const ConsultaAsignacion: React.FC = () => {
  ...
  console.log('🧪 [ConsultaAsignacion] Render del componente, filtros:', filtros, 'asignacionesLength:', asignaciones.length, 'loading:', loading);
  ...
  const handleSelectContribuyente = async (contribuyente: any) => {
    ...
    setFiltros({
      ...
      nombreContribuyente: contribuyente?.contribuyente || contribuyente?.nombreCompleto || ''
    });
  ...
  const handleBuscar = async () => {
    ...
    // Limpiar los filtros después de buscar (hace la función del botón Limpiar)
    setFiltros({
      anio: new Date().getFullYear().toString(),
      codigoContribuyente: '',
      nombreContribuyente: ''
          ...
          <Box sx={{ 
            ...
            <Box sx={{
              ...
              <TextField
                ...
                InputProps={{
                  readOnly: true,
                  startAdornment: <PersonIcon sx={{ mr: 0.5, color: 'action.active', fontSize: '1.1rem' }} />
                }}
                placeholder="---"
                sx={{
                  m: 0,
            ...
            <TableHead>
              <TableRow>
                ...
                {[
                  ...
                  { label: 'CÓD. PREDIO', width: 100, align: 'left' as const, icon: <HomeIcon fontSize="small" /> },
                  { label: 'CONTRIBUYENTE', width: 200, align: 'left' as const, icon: <PersonIcon fontSize="small" /> },
                  ...
                  { label: 'F. DECLARACIÓN', width: 110, align: 'center' as const },
                  ...
                  >
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5, justifyContent: col.align === 'center' ? 'center' : col.align === 'right' ? 'flex-end' : 'flex-start' }}>
                      {col.icon}
                      {col.label}
                    </Box>
                  </TableCell>
                ...
                {asignaciones.length === 0 ? (
                  ...
                  asignaciones.map((asignacion, index) => (
                    <Fade in={true} key={index} timeout={200 + (index * 50)}>
                      ...
                      >
                        {/* Año */}
                        <TableCell align="center" sx={{ py: 1.5, px: 1, borderBottom: `1px solid ${alpha(theme.palette.divider, 0.5)}` }}>
                          <Chip
                            label={asignacion.anio}
                            size="small"
                            variant="outlined"
                        ...
                        <TableCell sx={{ py: 1.5, px: 1, borderBottom: `1px solid ${alpha(theme.palette.divider, 0.5)}`, maxWidth: 200 }}>
                        ...
                        {/* Dirección */}
                        <TableCell sx={{ py: 1.5, px: 1, borderBottom: `1px solid ${alpha(theme.palette.divider, 0.5)}`, maxWidth: 250 }}>
                        ...
                        {/* Autoavalúo */}
                        <TableCell align="right" sx={{ py: 1.5, px: 1, borderBottom: `1px solid ${alpha(theme.palette.divider, 0.5)}` }}>
                          <Typography variant="body2" sx={{ fontSize: '0.75rem', fontWeight: 600, color: theme.palette.success.main }}>
                            S/ {asignacion.autoavaluo?.toLocaleString('es-PE', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) || '0.00'}
                          </Typography>
                        </TableCell>
                        ...
                        <TableCell align="right" sx={{ py: 1.5, px: 1, borderBottom: `1px solid ${alpha(theme.palette.divider, 0.5)}` }}>
                          <Typography variant="body2" sx={{ fontSize: '0.75rem', fontWeight: 500 }}>
                            S/ {asignacion.baseImponible?.toLocaleString('es-PE', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) || '0.00'}
                          </Typography>
                        </TableCell>

                        {/* Impuesto Anual */}
                        <TableCell align="right" sx={{ py: 1.5, px: 1, borderBottom: `1px solid ${alpha(theme.palette.divider, 0.5)}` }}>
                          <Typography variant="body2" sx={{ fontSize: '0.75rem', fontWeight: 600, color: theme.palette.error.main }}>
                            S/ {asignacion.impuestoAnual?.toLocaleString('es-PE', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) || '0.00'}
                          </Typography>
                        </TableCell>
                        ...
                        <TableCell align="center" sx={{ py: 1.5, px: 1, borderBottom: `1px solid ${alpha(theme.palette.divider, 0.5)}` }}>
                          <Stack direction="row" spacing={1} justifyContent="center">
                            <Tooltip title={`Editar asignación del predio ${asignacion.codPredio}`} arrow>
                              ...
                              >
                                <EditIcon fontSize="small" sx={{ color: theme.palette.primary.main, fontSize: '1rem' }} />
                              </IconButton>
                            </Tooltip>
                            <Tooltip title={`Desasignar predio ${asignacion.codPredio}`} arrow>
                              ...
                              >
                                <PersonRemoveIcon fontSize="small" sx={{ color: theme.palette.error.main, fontSize: '1rem' }} />
                              </IconButton>
                            </Tooltip>
                          </Stack>
                        </TableCell>
