 ...
 */
export interface CrudEntityActions<T, CreateDTO, UpdateDTO, F = Record<string, unknown>> {
  // Operaciones CRUD
...
>(
  ...
  // Estados
  const [state, setState] = useState<CrudEntityState<T, F>>({
    items: [],
    selectedItem: null,
    loading: false,
    creating: false,
  ...
  const loadItems = useCallback(async (params?: Record<string, unknown>) => {
    try {
      ...
      const items = await service.getAll(queryParams);
      
      // Aplicar filtro local si existe
      let filteredItems = localFilter && (state.searchTerm || Object.keys(state.filters as object).length > 0)
      ...
      const errorMessage = err.message || `Error al cargar ${entityName}s`;
      setState(prev => ({ ...prev, loading: false, error: errorMessage }));
      
      onError?.('load', err);
      console.error(`Error cargando ${entityName}s:`, error);
    }
  }, [service, state.page, state.pageSize, state.searchTerm, state.filters, localFilter, sortFunction, entityName, onError]);
  ...
  const createItem = useCallback(async (data: CreateDTO): Promise<T | null> => {
    try {
      ...
      const err = error as Error;
      const errorMessage = err.message || `Error al crear ${entityName}`;
  ...
  const updateItem = useCallback(async (id: string | number, data: UpdateDTO): Promise<T | null> => {
    try {
      ...
      const err = error as Error;
      const errorMessage = err.message || `Error al actualizar ${entityName}`;
  ...
  const deleteItem = useCallback(async (id: string | number): Promise<boolean> => {
    try {
      ...
      const err = error as Error;
      const errorMessage = err.message || `Error al eliminar ${entityName}`;
  ...
  const getItemById = useCallback(async (id: string | number): Promise<T | null> => {
    try {
      ...
      const err = error as Error;
      const errorMessage = err.message || `Error al obtener ${entityName}`;
      setState(prev => ({ ...prev, loading: false, error: errorMessage }));
  ...
  const clearFilters = useCallback(() => {
    setState(prev => ({ ...prev, filters: {} as F, searchTerm: '', page: 1 }));
  }, []);
  ...
  // Acciones
  const actions: CrudEntityActions<T, CreateDTO, UpdateDTO, F> = {
    loadItems,
    createItem,
    updateItem,
    deleteItem,
