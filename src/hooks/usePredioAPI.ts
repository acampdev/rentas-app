...
import { Predio, FiltroPredio, PredioFormData } from "../models/Predio";
...
const mapPredioDataToModel = (data: PredioData): Predio => {
  return {
    ...
    codClasificacion: data.codClasificacion
      ? Number(data.codClasificacion)
      : undefined,
    estPredio: data.estPredio,
    codTipoPredio: data.codTipoPredio ? Number(data.codTipoPredio) : undefined,
    codCondicionPropiedad: data.codCondicionPropiedad
      ? Number(data.codCondicionPropiedad)
      : undefined,
    ...
    codListaConductor: data.codListaConductor
      ? Number(data.codListaConductor)
      : undefined,
    codUbicacionAreaVerde: data.codUbicacionAreaVerde
      ? Number(data.codUbicacionAreaVerde)
      : undefined,
    codEstado: data.codEstado ? Number(data.codEstado) : undefined,
    codUsuario: data.codUsuario,

    numeroCondominos: data.numeroCondominos
      ? Number(data.numeroCondominos)
      : undefined,
  };
};
...
export const usePredios = (
  filtrosIniciales: PredioQueryFilters = { isAll: true },
) => {
  const queryClient = useQueryClient();
  const [filtrosBusqueda, setFiltrosBusqueda] = useState<PredioQueryFilters>(
    () => filtrosIniciales,
  );
  ...
  const {
    ...
    // Los valores calculados del predio pueden cambiar desde Pisos,
    // Asignación u otros módulos. No reutilizar durante cinco minutos la
    // caché global: al volver a la consulta se debe leer el estado del API.
    staleTime: 0,
  ...
  const mutationCrear = useMutation({
    mutationFn: async (
      ...
      const codClasificacionValue = String(
        datos.codClasificacion || datos.clasificacionPredio || "0502",
      ).trim();
      const esCasaHabitacion = codClasificacionValue === "0501";
      const codUsoPredioValue = esCasaHabitacion
        ? null
      ...
      const datosApi: CreatePredioDTO = {
        ...
        estPredio: String(
          datos.estPredio || datos.estadoPredio || "2503",
        ).trim(),
        codTipoPredio: String(
          datos.codTipoPredio || datos.tipoPredio || "2601",
        ).trim(),
        codCondicionPropiedad: String(
          datos.codCondicionPropiedad || datos.condicionPropiedad || "2701",
        ).trim(),
    ...
    onSuccess: (data) => {
      ...
      NotificationService.success(
        `Predio ${data.codPredio || "nuevo"} creado exitosamente`,
      );
    },
    onError: (err: Error) => {
  ...
  const buscarPrediosConFiltros = useCallback(
    (anio?: number, codPredioBase?: string, parametroBusqueda?: string) => {
      ...
      return queryClient.fetchQuery({
        ...
        queryFn: async () => {
          const data = await predioService.buscarPrediosConFiltros({
            anio,
            codPredioBase,
          });
          return data.map(mapPredioDataToModel);
