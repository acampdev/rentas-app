...
import { Box, Alert, Collapse, LinearProgress } from "@mui/material";
...
interface SavedContributorResult {
  persona: PersonaData;
...
const NuevoContribuyente: React.FC = () => {
    ...
    const cargarContribuyente = async () => {
      if (isEditMode && id) {
        try {
          ...
          logger.log(
            "📝 [NuevoContribuyente] Cargando contribuyente para edición, ID:",
            id,
          );

          // Enviar codigoContribuyente y codPersona como string vacío (no null)
          logger.log(
            "🚀 [NuevoContribuyente] Llamando a obtenerContribuyenteDetalle con codigoContribuyente:",
            id,
            'codPersona: ""',
          );
          ...
          logger.log(
            "📥 [NuevoContribuyente] Respuesta de obtenerContribuyenteDetalle:",
            detalle,
          );
          logger.log(
            "📥 [NuevoContribuyente] Tipo de detalle:",
            typeof detalle,
          );
          logger.log("📥 [NuevoContribuyente] Es null?:", detalle === null);
          logger.log(
            "📥 [NuevoContribuyente] Es array?:",
          ...
          if (Array.isArray(detalle)) {
            logger.log(
              "⚠️ [NuevoContribuyente] detalle es un ARRAY, extrayendo primer elemento",
            );
            detalleObj = detalle.length > 0 ? detalle[0] : null;
            logger.log(
              "📥 [NuevoContribuyente] Primer elemento extraído:",
              detalleObj,
            );
          }
          ...
          if (detalleObj) {
            logger.log(
              "✅ [NuevoContribuyente] ========== PROCESANDO DATOS ==========",
            );
            logger.log(
              "✅ [NuevoContribuyente] Datos del contribuyente cargados:",
              detalleObj,
            );
            logger.log(
              "🔍 [NuevoContribuyente] Campos disponibles:",
              Object.keys(detalleObj),
            );
            ...
            if (detalleObj.fechanacimiento || detalleObj.fechaNacimientoStr) {
              const fechaStr =
                detalleObj.fechaNacimientoStr || detalleObj.fechanacimiento;
            ...
            const convertirTipoDocumento = (codApi: string | null): string => {
              ...
              switch (codigo) {
                ...
                case "3":
                  return "4103"; // CE
                default:
                  return codigo.length === 4 ? codigo : "4101";
              }
            };
            ...
            const convertirSexo = (codApi: string | null): string => {
              ...
              switch (codigo) {
                case "1":
                  return "2001"; // Masculino
                case "2":
                  return "2002"; // Femenino
                default:
                  return codigo.length === 4 ? codigo : "2001";
              }
            };
            ...
            const convertirEstadoCivil = (codApi: string | null): string => {
              ...
              switch (codigo) {
                ...
                case "3":
                  return "1803"; // Viudo
                case "4":
                  return "1804"; // Divorciado
                default:
                  return codigo.length === 4 ? codigo : "1801";
              }
            };
            ...
            const extraerLoteDeDireccion = (
              direccionCompleta: string,
            ): { direccion: string; lote: string } => {
              if (!direccionCompleta) return { direccion: "", lote: "" };

              logger.log(
                "🔍 [extraerLoteDeDireccion] Procesando:",
                direccionCompleta,
              );
              ...
              if (match) {
                ...
                const direccionSinLote = direccionCompleta
                  .replace(regexLote, "")
                  .trim();
                logger.log("📍 [extraerLoteDeDireccion] Lote extraído:", lote);
                logger.log(
                  "📍 [extraerLoteDeDireccion] Dirección sin lote:",
                  direccionSinLote,
                );
              ...
              logger.log(
                "⚠️ [extraerLoteDeDireccion] No se encontró patrón Lt./Lote",
              );
            ...
            logger.log(
              "📍 [NuevoContribuyente] ========== PROCESANDO DIRECCIÓN ==========",
            );
            logger.log(
              "📍 [NuevoContribuyente] Dirección original del API:",
              direccionOriginal,
            );

            const { direccion: direccionLimpia, lote: loteExtraido } =
              extraerLoteDeDireccion(direccionOriginal);

            // Usar el lote del API si existe, sino usar el extraído de la dirección
            const nFincaFinal = detalleObj.lote?.trim() || loteExtraido || "";

            logger.log(
              "📍 [NuevoContribuyente] Dirección limpia (sin lote):",
              direccionLimpia,
            );
            ...
            const datosFormulario = {
              tipoDocumento: convertirTipoDocumento(
                detalleObj.codTipoDocumento,
              ),
              ...
              razonSocial:
                detalleObj.codTipopersona === "0302"
                  ? detalleObj.nombres?.trim()
                  : "",
            ...
            logger.log(
              "📋 [NuevoContribuyente] ========== DATOS MAPEADOS ==========",
            );
            logger.log(
              "📋 [NuevoContribuyente] Datos mapeados para formulario:",
              datosFormulario,
            );
            logger.log(
              "📋 [NuevoContribuyente] numeroDocumento:",
              datosFormulario.numeroDocumento,
            );
            logger.log(
              "📋 [NuevoContribuyente] nombres:",
              datosFormulario.nombres,
            );
            logger.log(
              "📋 [NuevoContribuyente] apellidoPaterno:",
              datosFormulario.apellidoPaterno,
            );
            logger.log("📋 [NuevoContribuyente] Llamando setInitialData...");
            setInitialData(datosFormulario);
            logger.log(
              "✅ [NuevoContribuyente] setInitialData ejecutado correctamente",
            );
          } else {
          ...
          logger.error(
            "❌ [NuevoContribuyente] Error al cargar contribuyente:",
            err,
          );
          setError(
            err instanceof Error
              ? err.message
              : "Error al cargar los datos del contribuyente",
          );
  ...
  const breadcrumbItems: BreadcrumbItem[] = [
    ...
    {
      label: isEditMode ? "Editar contribuyente" : "Nuevo contribuyente",
      active: true,
    },
  ];
  ...
  const handleSubmit = useCallback(
    async (resultado: SavedContributorResult) => {
      try {
        logger.log(
          "✅ [NuevoContribuyente] Contribuyente guardado exitosamente:",
          resultado,
        );

        // Marcar el listado como obsoleto antes de montar la consulta.
  ...
  return (
    <MainLayout
      title={isEditMode ? "Editar Contribuyente" : "Nuevo Contribuyente"}
    >
      <Box sx={{ p: 3 }}>
